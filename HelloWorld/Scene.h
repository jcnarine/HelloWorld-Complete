...
inline void from_json(const nlohmann::json& j, Scene& scene)
{
	...
	if (scrollHori)
	{
		...
		reg.get<HorizontalScroll>(EntityIdentifier::MainCamera()).SetFocus(&reg.get<Camera>(EntityIdentifier::MainPlayer()));
	...
	if (scrollVert)
	{
		...
		reg.get<VerticalScroll>(EntityIdentifier::MainCamera()).SetFocus(&reg.get<Camera>(EntityIdentifier::MainPlayer()));
